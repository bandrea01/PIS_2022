...
public class ManageStorePanel extends JPanel {
    public ManageStorePanel(MainLayout window) {
        ...
        if (!MagazzinoDAO.getInstance().existForPunto(punto)) {
            ...
            gridPanel.add(prodottoLabel); gridPanel.add(prodottiChooses);
            gridPanel.add(quantitaLabel); gridPanel.add(quantita);

            south.add(ButtonCreator.createButton("Confirm", true, ButtonCreator.LILLE, e -> MagazzinoBusiness.getInstance().rifornisciProdotto(prodottiChooses.getSelectedItem().toString(), Integer.parseInt(quantita.getSelectedItem().toString()), manager), null));
